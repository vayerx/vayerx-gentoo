...
KEYWORDS="~amd64 ~x86"
IUSE="berkdb debug doc examples gdbm ipv6 jemalloc +rdoc rubytests socks5 ssl tk xemacs ncurses +readline sse2"
...
src_configure() {
	...
	if ! use tk ; then
		modules="${modules}${modules:+,}tk"
	fi
	if ! use readline ; then
		modules="${modules}${modules:+,}readline"
	fi
	if ! use berkdb ; then
		modules="${modules}${modules:+,}dbm"
	fi
	if ! use gdbm ; then
		modules="${modules}${modules:+,}gdbm"
	fi
	if ! use ssl ; then
		modules="${modules}${modules:+,}openssl"
	fi
	if ! use ncurses ; then
		modules="${modules}${modules:+,}curses"
	fi
